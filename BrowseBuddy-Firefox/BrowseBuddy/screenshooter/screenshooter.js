document.addEventListener('DOMContentLoaded', function () {
    ...
    // Capture visible tab using Firefox API
    browser.tabs.captureVisibleTab(null).then((screenshotUrl) => {
      if (screenshotUrl) {
        img.src = screenshotUrl; // Use the captured screenshot URL
        img.onload = function () {
          const scale = window.devicePixelRatio;
          canvas.width = img.width / scale; // Set canvas width
          canvas.height = img.height / scale; // Set canvas height
          ctx.drawImage(img, 0, 0, img.width / scale, img.height / scale); // Draw the image on the canvas
        };
      }
    }).catch((error) => {
      console.error('Error capturing tab:', error); // Handle errors
    });
