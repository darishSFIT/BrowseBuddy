...
// Load data from Firefox storage
browser.storage.local.get(['websiteVisits']).then(result => { // Changed chrome to browser
...
function trackWebsiteTime() {
    ...
    if (currentSite && startTime) {
        ...
        browser.storage.local.set({ websiteVisits: websiteVisits }); // Changed chrome to browser
    }
    updateCurrentSite();
    startTime = now;
}
...
function updateCurrentSite() {
    console.log(browser.tabs); // Check if browser.tabs is defined
    browser.tabs.query({active: true, currentWindow: true}).then(tabs => { // Changed chrome to browser
...
browser.tabs.onActivated.addListener(updateCurrentSite); // Changed chrome to browser
