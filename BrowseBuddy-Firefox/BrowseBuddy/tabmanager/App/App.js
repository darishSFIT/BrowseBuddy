...
const App = (function() {
  ...
  function getTabsGroups() {
    if (browser.tabGroups) {
      return new Promise(resolve => {
        browser.tabGroups.query({}).then(tabGroup => {
          resolve(tabGroup);
        });
      });
    } else {
  ...
  function getTabsList(showOnlyCurrentWindow = false) {
    return new Promise(resolve => {
      if (showOnlyCurrentWindow) {
        ...
        browser.windows.getAll({ populate: true }).then(listOfWindows => {
  ...
  function onTabGroupClick(event) {
    ...
    if (target.dataset.type === 'group') {
      ...
      browser.tabGroups.get(groupId).then(tabGroup => {
  ...
  async function buildTabRow({ tab, currentWindowId, onlyTabInWindow, groupParams }) {
    ...
    // if (false && browser.tabGroups) {
    //     const { groupId } = tab; 

    //     if (groupId > 0) {
    //       const { tabColor, id, collapsed, title } = await (() => {
    //         return new Promise(resolve => {
    //           browser.tabGroups.get(groupId).then(tabGroupParams => {
  ...
  function toggleMute(tabId) {
    browser.tabs.get(tabId).then(tabData => {
